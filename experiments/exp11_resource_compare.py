...
def run_comparison(signal_type='slow_varying', n_templates=2000, n_test=60,
    ...
    timings = {m: {'retrieve': [], 'recon': []} for m in methods}
    ...
    for _ in tqdm(range(n_test), desc=f'  {signal_type}'):
        ...
        for mname, mtype in methods.items():
            ...
            timings[mname]['retrieve'].append(time.perf_counter() - t0)
            ...
            for n_bits in bit_levels:
                ...
                timings[mname]['recon'].append(time.perf_counter() - t0)
...
if __name__ == '__main__':
    ...
    for st in ['slow_varying', 'periodic', 'transient']:
        results, bw, n_s = run_comparison(
            st, n_templates=args.templates, n_test=args.test)
        all_data[st] = (results, bw, n_s)

    plot_resource_comparison(all_data)
