#!/usr/bin/env python
"""Atomically consume the single C1 temporal-confirmation access."""
...
def main() -> None:
    ...
    parser.add_argument("--state", type=Path, default=PROJECT_DIR / "configs/stage4_c1_temporal_access_state.json")
    ...
    if state.get("status") != "not_accessed" or state.get("access_count") != 0 or state.get("receipt") is not None:
        raise RuntimeError("temporal confirmation access is not available")
    snapshot = json.loads(args.snapshot.read_text(encoding="utf-8"))
    if canonical_json_sha256(snapshot.get("files", [])) != snapshot.get("executable_snapshot_sha256"):
        raise ValueError("executable snapshot digest mismatch")
