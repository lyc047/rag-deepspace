#!/usr/bin/env python
"""Atomically consume the one Stage-5 external Final access before signal read."""
...
def consume(
    *,
    ...
    if (
        ...
        raise RuntimeError("Stage-5 external Final access is not pristine")
    snapshot_errors = verify_code_snapshot(PROJECT_DIR, snapshot)
    if snapshot_errors:
        raise ValueError("frozen code changed: " + "; ".join(snapshot_errors))
    catalog_errors = validate_external_final_catalog(catalog, protocol, registry)
    if catalog_errors:
    ...
    return {
        ...
        "consumed_at": receipt["consumed_utc"],
        "actor": actor,
        "receipt": receipt,
...
def main() -> None:
    ...
    try:
        ...
        raise RuntimeError(
            "consume lock exists; access may be consumed or a prior run crashed; "
            "automatic reset is forbidden"
        ) from exc
    try:
        ...
        # Deliberately retain the lock. Human audit is required and reset is
        # never automated after a consumption attempt begins.
        raise
    print(
        json.dumps(
            {
