#!/usr/bin/env python
"""Plot Stage-9.1 hybrid watchdog confirmation results."""
...
def main() -> None:
    ...
    labels = ("Fixed-10", "Boot event", "Event + WD30", "Event + WD60")
    colors = ("#64748b", "#0f766e", "#2563eb", "#d97706")
    ...
    for index, (policy, label, color) in enumerate(zip(policies, labels, colors)):
        ...
        clean = [
            100.0 * payload["profiles"][name]["policies"][policy]["clean_rate"]["mean"]
            for name in profiles
        ]
    ...
    fig.suptitle("Stage-9.1 independent-seed hybrid-watchdog confirmation")
